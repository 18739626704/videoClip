...
1. 选择并预览视频，拖动到想要开始推流的时间点
2. 点击"开始推流"按钮
3. 复制 RTSP 地址（如 `rtsp://192.168.1.100:8554/live`）
4. 在 VLC 或其他播放器中打开该地址

**注意**：推流与预览独立运行。如需同步时间，点击"同步时间"按钮（会导致播放器短暂中断重连）。
...
### Q: RTSP 推流时 VLC 无法播放
A: 确保 MediaMTX 已正确配置并启动。检查防火墙是否放行了对应端口。
